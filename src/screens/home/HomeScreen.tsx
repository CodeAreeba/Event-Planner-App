...
const HomeScreen: React.FC = () => {
    ...
    const [bookingStats, setBookingStats] = useState<BookingStats>({ total: 0, upcoming: 0, completed: 0 });
    ...
    const loadData = async () => {
        ...
        if (user) {
            const { success: bookingSuccess, bookings } = await getBookings({ userId: user.uid });
            if (bookingSuccess && bookings) {
                const now = new Date();
                const upcoming = bookings.filter(b =>
                    new Date(b.date) > now && (b.status === 'pending' || b.status === 'confirmed')
                ).length;
            ...
            <View className="bg-primary pt-12 pb-8 px-6">
                <View className="flex-row items-center justify-between mb-4">
                    <View className="flex-1">
                        ...
                        <Text className="text-white text-2xl font-bold">{userProfile?.name || 'User'}</Text>
                    </View>
            ...
            <View className="px-6 -mt-6 mb-6">
                <ScrollView horizontal showsHorizontalScrollIndicator={false} className="gap-x-3">
                    ...
                    <View className="bg-white rounded-2xl p-4 shadow-sm" style={{ width: 140 }}>
                        ...
                        <Text className="text-2xl font-bold text-gray-900">{bookingStats.upcoming}</Text>
                    ...
                    <View className="bg-white rounded-2xl p-4 shadow-sm" style={{ width: 140 }}>
                        ...
                        <Text className="text-2xl font-bold text-gray-900">{bookingStats.completed}</Text>
            ...
            <View className="px-6 mb-6">
                ...
                <View className="flex-row flex-wrap gap-3">
                    ...
                    >
                        ...
                        <Text className="text-gray-500 text-xs mt-1">Find perfect services</Text>
                    </TouchableOpacity>
                    ...
                    >
                        ...
                        <Text className="text-gray-500 text-xs mt-1">View & edit profile</Text>
                    </TouchableOpacity>
                </View>
            </View>
            ...
            {recentBookings.length > 0 && (
                <View className="px-6 mb-6">
                    <View className="flex-row justify-between items-center mb-4">
                        <Text className="text-gray-900 text-lg font-bold">Recent Bookings</Text>
                    ...
                    {recentBookings.map((booking) => (
                        ...
                        >
                            <View className="flex-row justify-between items-start mb-2">
                                <Text className="text-gray-900 font-semibold flex-1">{booking.serviceName}</Text>
                            ...
                            <View className="flex-row items-center">
                                ...
                                <Ionicons name="time-outline" size={14} color="#9CA3AF" className="ml-3" />
                                <Text className="text-gray-500 text-sm ml-1">{booking.time}</Text>
                            </View>
                        </TouchableOpacity>
                    ))}
                </View>
