...
import { acceptBooking, getBookings, rejectBooking } from '../../firebase/bookings';
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        ...
                        const { success, error } = await acceptBooking(booking.id, user.uid);
    ...
    const BookingCard = ({ booking }: { booking: Booking }) => (
        <View className="bg-white rounded-2xl shadow-md p-4 mb-4">
            ...
            <View className="flex-row items-center mb-2">
                ...
                <Text className="text-gray-600 text-sm ml-2">{formatDate(booking.date)}</Text>
            </View>

            <View className="flex-row items-center mb-3">
                <Ionicons name="time-outline" size={16} color="#6B7280" />
                <Text className="text-gray-600 text-sm ml-2">{formatTime(booking.time)}</Text>
            </View>
            ...
            {booking.status === 'rejected' && booking.rejectionReason && (
                <View className="bg-red-50 p-3 rounded-lg mb-3">
                    <Text className="text-red-600 text-xs font-semibold">Rejection Reason:</Text>
                    <Text className="text-red-600 text-xs mt-1">{booking.rejectionReason}</Text>
                </View>
            )}

            {/* Price and Actions */}
            <View className="flex-row justify-between items-center pt-3 border-t border-gray-200">
                <Text className="text-primary text-xl font-bold">{formatCurrency(booking.price)}</Text>
        ...
        <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
            ...
            <View className="bg-primary pt-4 pb-6 px-6">
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <View className="flex-row gap-2">
                        {(['all', 'pending', 'accepted', 'completed'] as FilterTab[]).map((filter) => (
                            <TouchableOpacity
                                key={filter}
                                onPress={() => setActiveFilter(filter)}
                                className={`px-4 py-2 rounded-full ${activeFilter === filter ? 'bg-white' : 'bg-white/20'
                                    }`}
                            >
                                <Text
                                    className={`text-sm font-semibold capitalize ${activeFilter === filter ? 'text-primary' : 'text-white'
                                        }`}
                                >
            ...
            >
                <View className="flex-1 justify-end bg-black/50">
                    <View className="bg-white rounded-t-3xl p-6">
                        <Text className="text-gray-900 text-xl font-bold mb-4">Reject Booking</Text>
